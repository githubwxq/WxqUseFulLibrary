...
package com.example.wxq.wxqusefullibrary.luban;
...
//        File cacheDir = context.getCacheDir();  // 缓存目录
//        if (cacheDir != null) {
...
//    }
    ...
    public static File  getPhotoCacheDir(Context context ,String cacheName) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            cacheFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "wxq",cacheName);
            if (!cacheFile.exists()) {
                cacheFile.mkdir();
            }
        } else {
            ...
            if (cacheFile != null) {
                File result = new File(cacheFile, cacheName);
                if (!result.mkdirs() && (!result.exists() || !result.isDirectory())) {
                    // File wasn't able to create a directory, or the result exists but not a directory
                    return null;
                }
