...
public class MyDbHelper {
    ...
    private MyDbHelper(Context ctx) {
        Context applicationContext = ctx.getApplicationContext(); //防止内存泄漏
    ...
    public synchronized static MyDbHelper getInstance(Context context) {
        ...
        if (openHelper == null) {
            openHelper = new MyDbHelper(applicationContext);
        }
        openHelper.open(); //打开数据库
        return openHelper;
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            db.execSQL(DBManager.CREATE_TAB_BOOK);
        }
...
//                }
    ...
    public synchronized MyDbHelper open() throws SQLException {
        if (mDbHelper == null) { //为空
            mDbHelper = new DatabaseHelper(mCtx);
        }
        try {
            mDb = mDbHelper.getWritableDatabase(); //不为空
        } catch (Exception e) {
            e.printStackTrace();
        }
        return this;
