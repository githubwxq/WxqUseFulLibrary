...
//    private int textColor = getResources().getColor(R.color.gray);
    ...
    private boolean isExpandable = false;//如果需要展开收起，会用到这个变量

    private int whichline = -1, start = -1, end = -1;//如果需要显示不同的颜色，会用到这些变量
    private int color = -1;
...
//        extraWidth = dip2px(context, 2);
        ...
        isMatchParent = typedArray.getBoolean(R.styleable.MTextView_isMatchParent, false);
        typedArray.recycle();
    }
            ...
            for (int j = 0; j < aContentList.line.size(); j++) {
                ...
                if (ob instanceof String) {
                    ...
                    if (whichline == i && j == 0 && start < end && start < str.length() && end < str.length()) {
                        paint.setColor(getCurrentTextColor());
                        try {
                            canvas.drawText(str.substring(start, end), realDrawedWidth, height + aContentList.height - paint.getFontMetrics().descent, paint);
                        } catch (StringIndexOutOfBoundsException e) {
                            canvas.drawText(str.substring(start, str.length() - 1), realDrawedWidth, height + aContentList.height - paint.getFontMetrics().descent, paint);
                        }
                        paint.setColor(color);
                        canvas.drawText(str.substring(end), realDrawedWidth + paint.measureText(str.substring(start, end)), height + aContentList.height - paint.getFontMetrics().descent, paint);
                    } else {
                        canvas.drawText(str, realDrawedWidth, height + aContentList.height - paint.getFontMetrics().descent, paint);
                    }
...
//        if (!isExpandable) {
        int cachedHeight = getCachedData(text.toString() + (isExpandable ? ";getMaxLine=" + maxLine : ""), width);
        if (cachedHeight > 0) {
            return cachedHeight;
        }
//        }
                        ...
                        if (lineList != null && !lineList.isEmpty()) {
                            if (lineList.get(lineList.size() - 1) instanceof String) {
                                ...
                                if (str.length() >= 2) {
                                    ...
                                    lineList.set(lineList.size() - 1, stringBuilder.toString());
                                } else {
                                    if (lineList.size() > 1) {
...
//		}
        ...
        measuredData.put(text.toString() + (isExpandable ? ";getMaxLine=" + maxLine : ""), cache);
    }

    public void setMText(CharSequence cs, int whichline, int start, int end, int color) {
