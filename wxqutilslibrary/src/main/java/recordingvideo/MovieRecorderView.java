...
/**
 * 视频播放控件  封装了控件播放初始化参数 录制取消录制 添加对应回调给上层
...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
   ...
   private ProgressBar progressBar;
    ...
    public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
        ...
        recordMaxTime = a.getInteger(R.styleable.MovieRecorderView_record_max_time, 20);//默认最大拍摄时间为10s
        ...
        surfaceHolder = surfaceView.getHolder();  // surfaceview表面控制器
...
//        params.setPictureSize(480, 720);//拍照保存比率
    ...
    private void createRecordDir() {
        File sampleDir = new File(Environment.getExternalStorageDirectory() + File.separator + "wxq/video/");
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        try {
...
//        LogUtil.e(LOG_TAG,"手机支持的最大像素supportedPictureSizes===="+sizePicture);
     ...
     */
    public void record(final OnRecordFinishListener onRecordFinishListener) {//onRecordFinishListener 回调参数
        this.onRecordFinishListener = onRecordFinishListener;
        createRecordDir(); // 创建文件
        try {
            //如果未打开摄像头，则打开
            if (!isOpenCamera)
                initCamera();
            ...
            timer.schedule(new TimerTask() {
                ...
                public void run() {
                   ...
                   progressBar.setProgress(timeCount);//设置进度条
                    ...
                    //达到指定时间，停止拍摄
                    if (timeCount == recordMaxTime) {  // 定时任务到达预定时间点结束
                        stop();
